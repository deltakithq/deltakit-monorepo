...
description: Display tool calls with loading states and results in your UI.
---

When your server performs tool calls (function calling), the assistant message contains `ToolCallPart` entries alongside text. This guide shows how to handle the `onEvent` callback and render tool calls in the UI.
