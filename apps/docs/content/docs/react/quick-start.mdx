...
If you need side-effects around that lifecycle, use `onStatusChange`:

```tsx
useStreamChat({
  api: "/api/chat",
  onStatusChange: (status, { reason, error }) => {
    if (status === "starting") console.log("Started");
    if (status === "stopped" && reason === "user") console.log("Stopped");
...
- [useStreamChat lifecycle callbacks](/docs/react/use-stream-chat#onstatuschange): Track start, resume, stop, finish, and error transitions
