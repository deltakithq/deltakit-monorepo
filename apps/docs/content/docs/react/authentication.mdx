...
description: Pass auth tokens and handle unauthorized errors.
---

DeltaKit doesn't have a built-in auth system. Authentication is handled through the `headers` option on `useStreamChat`.
