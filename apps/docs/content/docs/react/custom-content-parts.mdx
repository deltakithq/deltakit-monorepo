...
DeltaKit's built-in `ContentPart` union covers text, tool calls, and reasoning. When you need additional part types — images, status indicators, citations, or anything else — use the generic type parameter on `useStreamChat`.
