...
Buffers streamed `appendText` / `text_delta` updates and flushes them after the given number of incoming text chunks. This is useful when your backend emits very small deltas and you want fewer React re-renders while preserving the final message.
...
`debounced.tokens` counts incoming text-delta calls, which often map closely to streamed tokens depending on your backend. Any buffered text is always flushed before `finish`, `error`, `stop()`, or before a non-text part is appended.
...
**Type:** `(status: "starting" | "resuming" | "stopped" | "finished" | "error", context: { messages: Message<TPart>[]; runId: string | null; error?: Error; reason?: "user" | "unmount" }) => void`

Called whenever the stream lifecycle changes. Use this for analytics, toasts, logging, or side-effects that need more detail than `isLoading`.

```ts
useStreamChat({
  api: "/api/chat",
  onStatusChange: (status, { runId, reason, error }) => {
    if (status === "starting") {
...
- `starting` — a new `sendMessage()` request started
- `resuming` — the hook is reconnecting to an existing run
- `stopped` — the active run stopped because the user called `stop()` or the component unmounted
- `finished` — the stream completed successfully
