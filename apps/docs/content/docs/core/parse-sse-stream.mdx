...
```ts
import { parseSSEStream } from "@deltakit/react";
```
...
- You typically don't need to use `parseSSEStream` directly. The `useStreamChat` hook calls it internally.
- It is useful when building non-React integrations or when you need direct control over the stream outside of the hook.
